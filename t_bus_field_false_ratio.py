...
#    poly=[[ 113.7519, 22.7223 ], [ 113.8857, 22.4532 ], [ 114.4124, 22.6030 ], [ 114.5084, 22.4413 ], [ 114.6289, 22.5130 ]  ,[114.5034,22.5781],[114.6026,22.6536],[114.4383,22.6992],[114.4009,22.7930],[114.1824,22.8231],[114.1685,22.6853],[113.8716,22.8634],[ 113.7519, 22.7223 ]]
    ...
    cursor.execute("select table_catalog,table_schema,table_type,table_name,field,type,varchar_maxlength,comment from bigdata.t_table_field1 where table_name like 't_bus_%' and comment='公交线路名称' or comment='线路（不分方向）名称' or comment='线路（分方向）编号' or comment='站点编号' or comment='断面字段';")
    table_value = cursor.fetchone()
    sum_false=0
    sum_total=0
    if table_value[7]=="公交线路名称":
        if table_value[4] !="line_name": comment="字段名不匹配"

        if table_value[5] != "varchar": comment="字段类型不匹配"

        if table_value[6] !="32": comment="字段长度不匹配"

    elif table_value[7]=="线路（不分方向）名称":
        if table_value[4] !="route_name": comment="字段名不匹配"

        if table_value[5] != "varchar": comment="字段类型不匹配"

        if table_value[6] !="100": comment="字段长度不匹配"

    elif table_value[7] == "线路（分方向）编号":
        if table_value[4] != "route_id": comment="字段名不匹配"

        if table_value[5] != "varchar": comment="字段类型不匹配"

        if table_value[6] != "32": comment="字段长度不匹配"

    elif table_value[7] == "站点编号":
        if table_value[4] != "stop_id": comment="字段名不匹配"

        if table_value[5] != "varchar": comment="字段类型不匹配"

        if table_value[6] != "32": comment="字段长度不匹配"

    elif table_value[7] == "断面字段":
        if table_value[4] != "stop_name": comment="字段名不匹配"
...
#        GROUP BY a.table_catalog,a.table_schema,a.table_type, a.table_name,a.field,b.field,c.field'''
    ...
    for i in list2:
        ...
        parm = (str(i[0]), str(i[1]),str(i[2]),str(i[3]), str(sum_false), str(sum_false/sum_total), date,sum_total)
        print(parm)
